...
def check_filename_existence(func):
    def inner(*args, **kwargs):
        if not os.path.exists(args[1]):
            raise BabynameFileNotFoundException(('No such babyname file or directory: {0}'.format(args[1])))
...
class BabynameParser:
    ...
    def __init__(self, filename):
        ...
        text = r.read()  # TODO: Open and read the given file.
        ...
        self.rank_to_names_tuples = re.findall("<td>([0-9]+)</td><td>([A-Z][a-z]+)</td><td>([A-Z][a-z]+)</td>",text)
        # TODO: Extract the list of rank to names tuples.


    def parse(self, parsing_lambda):
        parsed_name = []
        for names in self.rank_to_names_tuples:
            parsed_name.append(parsing_lambda(names))
